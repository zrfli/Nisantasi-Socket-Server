...
io.on('connection', (socket) => {
    ...
    // Remove previous student's attendance info if any
    Object.keys(teacherCodes).forEach((key) => {
        if (teacherCodes[key].studentId === studentId) {
            delete teacherCodes[key];
        }
    });

    // Mark the student's attendance code as verified with the new code
    teacherCodes[code] = { teacherId, studentId };
